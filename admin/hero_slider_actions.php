...
$redirect_url = ADMIN_URL . '/hero_sliders.php'; // Default redirect
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'add_edit') {
    ...
    if (empty($title) || empty($image_url)) {
        ...
        if ($id) {
            header('Location: ' . ADMIN_URL . '/hero_slider_form.php?id=' . $id);
        } else {
            header('Location: ' . ADMIN_URL . '/hero_slider_form.php');
        }
        exit;
    }
